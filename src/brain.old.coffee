...
class Brain extends EventEmitter
  ...
  lpop: (key) ->
    Q(@deserialize(@_data.get(@key(key))?.shift()))

  # pop a value off of the right-side of the list
  #
  # Returns promise for list item
  rpop: (key) ->
    Q(@deserialize(@_data.get(@key(key))?.pop()))

  # get a list item by index
  #
  # Returns promise for list item
  lindex: (key, index) ->
    Q(@deserialize(@_data.get(@key(key))?[index] or null))
  ...
  keys: (searchKey = '') ->
    ...
    Q(_.map(_.filter(@_data.keys(), (key) -> key.indexOf(searchKey) is 0), @unkey.bind(@)))
  ...
  # Returns promise
  set: (key, value) ->
    @_data.set(@key(key), @serialize(value))
    Q()
  ...
  # Returns promise
  get: (key) ->
    Q(@deserialize(@_data.get(@key(key)) ? null))

  # Get the type of the value at `key`
  #
  # Returns promise
  type: (key) ->
    val = @deserialize(@_data.get(@key(key)))
  ...
  incrby: (key, num) ->
    key = @key(key)
    @_data.set(key, (@_data.get(key) or 0) + num)
    Q(@_data.get(key))
  ...
  hgetall: (table) ->
    Q(new Map(@_data.get(@key(table))?.entries() or null))
  ...
  userForName: (name) ->
    ...
    user = _.find @_users.values(), (user) ->
  ...
  usersForRawFuzzyName: (fuzzyName) ->
    ...
    users = _.filter @_users.values(), (user) ->
